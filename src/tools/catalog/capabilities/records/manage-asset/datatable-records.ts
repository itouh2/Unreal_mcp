...
// The native handler reads `save` on all four write actions
// (GetPayloadBool -> McpSafeAssetSave) but no record ever declared it, so
// the gateway refused it as UNDECLARED_PARAMETER and every table and row
// struct authored over MCP stayed in memory and died with the editor.
const SAVE = bool('Persist the asset to disk. Defaults to false, which leaves it in memory only.');
...
export const DATATABLE_RECORDS: readonly RecordSpec[] = [
  r('create_data_table', 'datatable', 'Create a new DataTable asset.', schema({ name: str('DataTable name.'), path: str('Package path.'), rowStructPath: ROW_STRUCT, save: SAVE }, ['name']), OK, WRITE, WRITE_POLICY, MEDIUM,
    { dispatchMode: 'tool', examples: [ex('Create a weapons DataTable', { name: 'DT_Weapons', path: '/Game/DataTables', rowStructPath: ROW }, DONE)] }),
  r('set_data_table_row_struct', 'datatable', 'Bind a row UScriptStruct to an existing DataTable.', schema({ dataTablePath: DT_PATH, rowStructPath: ROW_STRUCT, save: SAVE }, ['dataTablePath', 'rowStructPath']), OK, WRITE, WRITE_POLICY, LOW,
    { dispatchMode: 'tool', examples: [ex('Bind the row struct after creation', { dataTablePath: DT, rowStructPath: ROW }, DONE)] }),
  r('create_row_struct', 'datatable', 'Create a new UScriptStruct suitable as a DataTable row type.', schema({ name: str('Struct name.'), path: str('Package path.'), members: arrObj('Member definitions.'), save: SAVE }, ['name']), OK, WRITE, WRITE_POLICY, MEDIUM,
  ...
  r('set_struct_as_row_struct', 'datatable', 'Set an existing UScriptStruct as the row struct for a DataTable.', schema({ dataTablePath: DT_PATH, rowStructPath: ROW_STRUCT, save: SAVE }, ['dataTablePath', 'rowStructPath']), OK, WRITE, WRITE_POLICY, LOW,
