...
export const ANIM_AUTHORED_3: readonly CapabilityRecordSource[] = [
  ...
  buildRecord({ parentTool: T, id: `${T}.setup_retargeting`, action: 'setup_retargeting', family: F,
    summary: 'Retarget animation onto another skeleton through an IK Retargeter, baking new AnimSequences.', whenToUse: ['Cross-skeleton retarget needed.', 'Mocap or marketplace clips must play on a different rig.'], whenNotToUse: ['Use create_ik_retargeter to build the retargeter asset without baking.'],
    inputProps: { action: P.action, sourceSkeleton: P.skeletonPath, targetSkeleton: P.skeletonPath, assets: A.assets, savePath: A.savePath, suffix: A.suffix, overwrite: P.overwrite, sourceMesh: { type: 'string', description: 'SkeletalMesh giving the source proportions. Defaults to the skeleton preview mesh, then any mesh in the project built on it.' }, targetMesh: { type: 'string', description: 'SkeletalMesh to retarget onto. Defaults the same way as sourceMesh.' }, retargeterPath: { type: 'string', description: 'An existing IK Retargeter to bake through. Omit to auto-build one by characterizing both skeletons and mapping their chains.' } }, required: ['action'],
    ...
    exampleOutput: { success: true, message: 'Retargeting completed' } }),
  buildRecord({ parentTool: T, id: `${T}.skin_mesh_to_skeleton`, action: 'skin_mesh_to_skeleton', family: F,
    summary: 'Skin a static mesh to a skeleton so it deforms with the animation, producing a SkeletalMesh.', whenToUse: ['Clothing, armour or a prop must move with the body instead of holding its shape.', 'A garment downloaded as a static mesh needs to be worn.'], whenNotToUse: ['The mesh is already a SkeletalMesh on the right skeleton.'],
    inputProps: { action: P.action, staticMeshPath: { type: 'string', description: 'StaticMesh to skin, e.g. a coat downloaded as a rigid mesh.' }, skeletalMeshPath: { type: 'string', description: 'SkeletalMesh whose geometry to re-skin, for a dressed character whose own rig will not retarget. Its materials carry over. Use instead of staticMeshPath.' }, skeletonPath: P.skeletonPath, outputPath: { type: 'string', description: 'Canonical /Game path for the SkeletalMesh this writes.' },
      sourceSkeletalMesh: { type: 'string', description: 'An already-skinned mesh on the same skeleton to copy weights from, such as the body the garment is worn over. Beats computing weights fresh wherever the garment hugs the body. Falls back to smooth binding when omitted.' }, save: P.save }, required: ['action', 'skeletonPath', 'outputPath'], requiredOneOf: ['staticMeshPath', 'skeletalMeshPath'],
    effect: 'write', behavior: { idempotency: 'idempotent' }, latency: 'interactive', resources: 'medium', plugins: ESU, normalizationProvenance: 'post-migration',
    outputProps: { assetPath: P.assetPath, weights: { type: 'string', description: "Which binding ran: 'transferred' from sourceSkeletalMesh, or 'smooth'." } }, outputRequired: [],
    exampleInput: { action: 'skin_mesh_to_skeleton', staticMeshPath: '/Game/Fab/coat', skeletonPath: '/Game/Chars/SK_Hero_Skeleton', outputPath: '/Game/Chars/SKM_Coat', sourceSkeletalMesh: '/Game/Chars/SKM_Hero' },
    exampleOutput: { success: true, message: 'Mesh skinned to skeleton' } }),
