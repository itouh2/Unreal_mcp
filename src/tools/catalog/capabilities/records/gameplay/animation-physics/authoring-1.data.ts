...
export const ANIM_AUTHORED_1: readonly CapabilityRecordSource[] = [
  ...
  buildRecord({ parentTool: T, id: `${T}.add_state`, action: 'add_state', family: F,
    ...
    inputProps: { action: P.action, assetPath: P.assetPath, blueprintPath: P.blueprintPath, stateMachineName: A.stateMachineName, machineName: { type: 'string', description: 'Alias of stateMachineName. Omitting both matches every state machine in the graph.' }, stateName: P.stateName, animations: { type: 'array', items: { type: 'string' }, description: 'Animation asset paths to play in this state; one Sequence Player is created per entry and the first drives the state Result pose. Without it the state is created empty and the character holds its reference pose.' }, save: P.save }, required: ['action', 'stateName'],
    effect: 'write', latency: 'interactive', resources: 'low', plugins: ESU,
    outputProps: { blueprintPath: P.blueprintPath, machineName: P.machineName, stateName: P.stateName, animationsApplied: { type: 'array', items: { type: 'object', properties: { animationPath: { type: 'string' }, nodeId: { type: 'string' }, connected: { type: 'boolean' } }, required: ['animationPath', 'nodeId', 'connected'], additionalProperties: false }, description: 'One entry per created Sequence Player. Only the first can be connected to the state Result pose.' }, animationsFailed: { type: 'array', items: { type: 'string' }, description: 'Requested animation paths that could not be loaded.' } }, outputRequired: [],
    exampleInput: { action: 'add_state', blueprintPath: '/Game/ABP_Char', stateMachineName: 'SM_Loco', stateName: 'Idle', animations: ['/Game/Anims/A_Idle'] },
  ...
  buildRecord({ parentTool: T, id: `${T}.set_transition_rules`, action: 'set_transition_rules', family: F,
    summary: 'Set transition rule logic: the condition a transition tests, plus its blend timing.',
    whenToUse: ['Transition condition must change.', 'A state machine must actually be able to leave a state: author the condition here after add_transition.'],
    whenNotToUse: ['Use add_transition.'],
    inputProps: { action: P.action, assetPath: P.assetPath, blueprintPath: P.blueprintPath, stateMachineName: A.stateMachineName, fromState: P.fromState, toState: P.toState, blendTime: P.blendTime, crossfadeDuration: A.crossfadeDuration, priorityOrder: A.priorityOrder, automaticRule: A.automaticRule, bidirectional: A.bidirectional, conditionVariable: A.conditionVariable, conditionComparison: A.conditionComparison, conditionValue: A.conditionValue }, required: ['action'],
    outputProps: { condition: { type: 'string', description: 'The rule that was written into the transition graph. Absent means no conditionVariable was given, so the transition stays false and the state is never left.' } },
    effect: 'write', behavior: { idempotency: 'idempotent' }, latency: 'interactive', resources: 'low', plugins: ESU,
    exampleInput: { action: 'set_transition_rules', blueprintPath: '/Game/ABP_Char', stateMachineName: 'SM_Loco', fromState: 'Idle', toState: 'Run', conditionVariable: 'Speed', conditionComparison: 'greater', conditionValue: 10 },
    exampleOutput: { success: true, message: 'Transition rules updated', condition: 'Speed greater 10' } }),
  buildRecord({ parentTool: T, id: `${T}.delete_transition`, action: 'delete_transition', family: F,
    summary: 'Delete a transition from a State Machine.',
    whenToUse: ['A transition was authored between the wrong two states.', 'A transition is dead weight and its rule graph should go with it.'],
    whenNotToUse: ['The transition should stay but never fire: use set_transition_rules.', 'The state itself must go, not just the transition.'],
    inputProps: { action: P.action, assetPath: P.assetPath, blueprintPath: P.blueprintPath, stateMachineName: A.stateMachineName, fromState: P.fromState, toState: P.toState, save: P.save }, required: ['action'],
    outputProps: { deleted: { type: 'boolean', description: 'True when the transition and its rule graph were removed.' } },
    effect: 'write', behavior: { idempotency: 'non-idempotent' }, latency: 'interactive', resources: 'low', plugins: ESU, normalizationProvenance: 'post-migration',
    exampleInput: { action: 'delete_transition', blueprintPath: '/Game/ABP_Char', stateMachineName: 'SM_Loco', fromState: 'Jump', toState: 'Idle' },
    exampleOutput: { success: true, message: 'Transition deleted', deleted: true } }),
