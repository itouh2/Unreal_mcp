...
// Proves: identical TS/JSON pilot emits and hashes across repeated
...
describe('build_environment pilot hash parity (TS/JSON)', () => {
  it('two pilot emit calls produce identical JSON and TS bytes', () => {
  ...
  it('two pilot emit calls produce identical hashes for JSON and TS', () => {
    const records = BUILD_ENVIRONMENT_RECORDS.map((r) => createCapabilityRecord(r));
