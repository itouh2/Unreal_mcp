...
const testCases = [
  ...
  { scenario: 'ENUM: split_enum with an explicit split index', toolName: 'manage_asset', arguments: { action: 'split_enum', enumPath: '${captured:enumPath}', newEnumName: `E_MCP_EnumSplitIdx_${ts}`, values: ['Green'], index: 1, path: TEST_FOLDER, save: false }, expected: 'success', assertions: [{ path: 'structuredContent.result.valueCount', equals: 1, label: 'split enum carries the selected value' }] },
  ...
  { scenario: 'ENUM ERROR: get_enum on missing enum', toolName: 'manage_asset', arguments: { action: 'get_enum', enumPath: `${TEST_FOLDER}/E_MCP_NoSuchEnum_${ts}` }, expected: 'error', assertions: [{ path: 'structuredContent.error', includes: 'Enum not found', label: 'missing enum reported' }] },
